...
+ (NSString *)chineseCalenderStringWithDate:(NSDate *)date
{
    ...
    // 24节气(有些为活节不固定日期如清明、冬至等)
    LunarCalendar *lunarCalendar = [date chineseCalendarDate];
    if (lunarCalendar.SolarTermTitle.length > 0) {
        chineseCalenderString = [NSString stringWithFormat:@"%@", lunarCalendar.SolarTermTitle];
    }
    ...
    // 没有公历节日或者24节气则显示农历或农历节日
    //    chineseCalenderString = [[self class]chineseHolidayForString:lunarString];
    chineseCalenderString = [[self class]chineseHolidayForLunarMonth:lunarCalendar.MonthLunar day:lunarCalendar.DayLunar];
    return chineseCalenderString;
}
/**
...
/**
 * @brief 根据农历月日有农历节日返回农历节日字符串否则返回农历月日
 */
+ (NSString *)chineseHolidayForLunarMonth:(NSString *)lunarMonth day:(NSString *)lunarDay
{
    NSString *chineseHoliday;
    if ([lunarDay isEqualToString:@"初一"]) {
        chineseHoliday = lunarMonth;
    }
    else
    {
        chineseHoliday = lunarDay;
    }
    if([lunarMonth isEqualToString:@"正月"] &&
       [lunarDay isEqualToString:@"初一"]){
        
        //正月初一：春节
        chineseHoliday = @"春节";
        
    }else if([lunarMonth isEqualToString:@"正月"] &&
             [lunarDay isEqualToString:@"十五"]){
        
        
        //正月十五：元宵节
        chineseHoliday = @"元宵";
        
    }else if([lunarMonth isEqualToString:@"二月"] &&
             [lunarDay isEqualToString:@"初二"]){
        
        //二月初二：春龙节(龙抬头)
        //        chineseHoliday = @"龙抬头";
        
    }else if([lunarMonth isEqualToString:@"五月"] &&
             [lunarDay isEqualToString:@"初五"]){
        
        //五月初五：端午节
        chineseHoliday = @"端午";
        
    }else if([lunarMonth isEqualToString:@"七月"] &&
             [lunarDay isEqualToString:@"初七"]){
        
        //七月初七：七夕情人节
        chineseHoliday = @"七夕";
        
    }else if([lunarMonth isEqualToString:@"八月"] &&
             [lunarDay isEqualToString:@"十五"]){
        
        //八月十五：中秋节
        chineseHoliday = @"中秋";
        
    }else if([lunarMonth isEqualToString:@"九月"] &&
             [lunarDay isEqualToString:@"初九"]){
        
        //九月初九：重阳节、中国老年节（义务助老活动日）
        chineseHoliday = @"重阳";
        
    }else if([lunarMonth isEqualToString:@"腊月"] &&
             [lunarDay isEqualToString:@"初八"]){
        
        //腊月初八：腊八节
        //        chineseHoliday = @"腊八";
        
    }else if([lunarMonth isEqualToString:@"腊月"] &&
             [lunarDay isEqualToString:@"廿四"]){
